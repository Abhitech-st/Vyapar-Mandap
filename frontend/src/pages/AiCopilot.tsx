  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-card rounded-2xl flex flex-col h-[600px] overflow-hidden border border-slate-800">
          
          {/* Top Bar */}
          <div className="px-5 py-3.5 bg-slate-950 border-b border-slate-800 flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <Sparkles className="w-4 h-4 text-emerald-400" />
              <span className="font-semibold text-xs text-slate-200">Natural Language Financial Query Copilot</span>
            </div>
          ...
          <div className="flex-1 p-5 overflow-y-auto space-y-4">
            {messages.map((m, idx) => (
              ...
              >
                {m.sender === 'bot' && (
                  <div className="w-8 h-8 rounded-lg bg-emerald-950 border border-emerald-600 text-emerald-400 flex items-center justify-center shrink-0 text-xs">
            ...
            {isLoading && (
              <div className="text-xs text-emerald-400 font-mono animate-pulse flex items-center space-x-2">
          ...
          <div className="p-4 bg-slate-950 border-t border-slate-800 flex items-center space-x-3">
            ...
            <button
              onClick={handleSend}
              className="p-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white transition shadow-lg shadow-emerald-600/20"
            >
        ...
        <div className="glass-card rounded-2xl p-5 space-y-4 overflow-y-auto max-h-[600px] border border-slate-800">
          <div className="border-b border-slate-800 pb-3 flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <Workflow className="w-4 h-4 text-emerald-400" />
              <h3 className="font-semibold text-xs text-slate-200">Deployed AI Agents (10/10)</h3>
            </div>
          ...
          <div className="space-y-3">
            {agents.map((ag, idx) => (
              <div key={idx} className="bg-slate-900/90 border border-slate-800/90 rounded-xl p-3 space-y-1 hover:border-slate-700 transition">
                ...
                <p className="text-[11px] text-slate-400">{ag.desc}</p>
              </div>
            ))}
          </div>
        </div>
