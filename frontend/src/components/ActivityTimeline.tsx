    ...
    <div className="glass-card rounded-xl p-5 space-y-4">
      <div className="flex items-center justify-between border-b border-slate-200 pb-3">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="font-bold text-slate-800 text-sm">Agent Activity & Operational Trace</h3>
        </div>

        {/* Filter Toggle */}
        <div className="flex items-center bg-slate-100 border border-slate-200 p-0.5 rounded-lg text-xs font-semibold">
          <button
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-md transition text-[11px] flex items-center space-x-1 ${
              filterMode === 'exceptions' ? 'bg-amber-100 text-amber-800 border border-amber-300 font-bold' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
        ...
        {displayedActivities.map((act) => (
          <div key={act.id} className="flex items-start space-x-3 relative group">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs z-10 shrink-0 ${
              act.type === 'warning' ? 'bg-amber-50 border border-amber-400 text-amber-800 shadow-xs' :
              act.type === 'success' ? 'bg-emerald-50 border border-emerald-500 text-emerald-700' :
            ...
            <div className={`flex-1 rounded-lg p-3 border transition ${
              ...
              <div className="flex items-center justify-between text-xs mb-1">
                <span className="font-bold text-slate-800 flex items-center space-x-1.5">
                  <span>{act.agent}</span>
                  <span className="text-[11px] text-slate-500 font-mono">({act.step})</span>
                </span>
              ...
              <p className="text-xs text-slate-700 leading-relaxed font-medium">{act.message}</p>
