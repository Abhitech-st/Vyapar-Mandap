...
- Primitives & Layouts: [[UI Components]]
- Backend API Integration: [[API Contracts]]
- End-to-End User Journeys: [[User Flows]]
...
- **Middle Left**: Linear-style AI Agent Activity Ticker ([[UI Components]]).
...
- **Split-Screen Review Modal**: Document Viewer canvas on left; Extracted JSON fields + AI reasoning + proposed double-entry journal entry ($Dr. Expense / Cr. Vendor$) on right with 1-click **Approve & Post** ([[User Flows]]).
...
- Manual journal entry poster with live $Debits = Credits$ balance check indicator ([[Database Schema]]).
...
- Natural language AI Assistant chat interface ([[Prompts]]).
