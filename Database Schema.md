...
8. `invoices`: Header parsed invoice records (id, organization_id, vendor_id, invoice_number, subtotal, tax_total, grand_total, status, ai_confidence).
9. `invoice_items`: Invoice line items (id, invoice_id, description, hsn_sac_code, quantity, unit_price, tax_rate, total_amount).
10. `journal_entries`: Double-entry master headers (id, organization_id, invoice_id, entry_number, entry_date, narration, status, is_immutable).
...
17. `reports`: Cached P&L / Balance Sheet snapshots (id, organization_id, report_type, payload_json).
...
22. `ai_conversations`: RAG chat history and memory (id, organization_id, user_id, messages_json).
